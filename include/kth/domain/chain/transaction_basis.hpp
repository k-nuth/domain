...
public:
    ...
    static
    transaction_basis factory_from_data(data_chunk const& data, bool wire = true, bool witness = false);
    
    static
    transaction_basis factory_from_data(std::istream& stream, bool wire = true, bool witness = false);

    template <typename R, KTH_IS_READER(R)>
    static
    transaction_basis factory_from_data(R& source, bool wire = true, bool witness = false) {
