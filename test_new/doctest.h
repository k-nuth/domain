...
#define DOCTEST_STR_CONCAT_TOSTR(s1, s2) DOCTEST_TOSTR(s1) \
DOCTEST_TOSTR(s2)
...
template <>
struct Types<> { typedef detail::NullType Result; };
...
struct DOCTEST_INTERFACE Result {
    ...
    DOCTEST_NOINLINE Result(const Result& x)
        : m_passed(x.m_passed), m_decomposition(x.m_decomposition) {}
...
DOCTEST_INTERFACE void logAssert(bool passed, const char* decomposition, bool threw, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line);

DOCTEST_INTERFACE void logAssertThrows(bool threw, const char* expr, assertType::Enum assert_type, const char* file, int line);

DOCTEST_INTERFACE void logAssertThrowsAs(bool threw, bool threw_as, const char* as, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line);

DOCTEST_INTERFACE void logAssertNothrow(bool threw, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line);
...
struct DOCTEST_INTERFACE ResultBuilder {
    ...
    ResultBuilder(assertType::Enum assert_type, const char* file, int line, const char* expr, const char* exception_type = "");
...
template <int comparison, typename L, typename R>
DOCTEST_NOINLINE int fast_binary_assert(assertType::Enum assert_type, const char* file, int line, const char* expr, const DOCTEST_REF_WRAP(L) lhs, const DOCTEST_REF_WRAP(R) rhs) {
...
template <typename L>
DOCTEST_NOINLINE int fast_unary_assert(assertType::Enum assert_type, const char* file, int line, const char* val_str, const DOCTEST_REF_WRAP(L) val) {
...
class ContextBuilder {
    ...
    DOCTEST_NOINLINE ContextBuilder(ContextBuilder& x)
        : numCaptures(x.numCaptures), head(x.head), tail(x.tail) {
        x.numCaptures = 0;
        x.head = 0;
        x.tail = 0;
...
namespace doctest {
namespace detail {
TestCase::TestCase(funcType test, const char* file, unsigned line, const TestSuite& test_suite, const char* type, int template_id)
    : m_test(test), m_name(0), m_type(type), m_test_suite(test_suite.m_test_suite), m_description(test_suite.m_description), m_skip(test_suite.m_skip), m_may_fail(test_suite.m_may_fail), m_should_fail(test_suite.m_should_fail), m_expected_failures(test_suite.m_expected_failures), m_timeout(test_suite.m_timeout), m_file(file), m_line(line), m_template_id(template_id) {}
...
Subcase::Subcase(const Subcase& x)
    : m_signature(x.m_signature.m_name, x.m_signature.m_file, x.m_signature.m_line), m_entered(x.m_entered) {}
...
#ifdef DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
void toStream(std::ostream* stream, char* in) {
    *stream << in;
}
void toStream(std::ostream* stream, const char* in) {
    *stream << in;
}
#endif  // DOCTEST_CONFIG_TREAT_CHAR_STAR_AS_STRING
...
void toStream(std::ostream* stream, char signed in) {
    *stream << in;
}
void toStream(std::ostream* stream, char unsigned in) {
    *stream << in;
}
void toStream(std::ostream* stream, int short in) {
    *stream << in;
}
void toStream(std::ostream* stream, int short unsigned in) {
    *stream << in;
}
void toStream(std::ostream* stream, int in) {
    *stream << in;
}
void toStream(std::ostream* stream, int unsigned in) {
    *stream << in;
}
void toStream(std::ostream* stream, int long in) {
    *stream << in;
}
void toStream(std::ostream* stream, int long unsigned in) {
    *stream << in;
}

#ifdef DOCTEST_CONFIG_WITH_LONG_LONG
void toStream(std::ostream* stream, int long long in) {
    *stream << in;
}
...
#else
// TODO(libbitcoin): integration with XCode and other IDEs
...
void logAssert(bool passed, const char* decomposition, bool threw, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line) {
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
...
void logAssertThrowsAs(bool threw, bool threw_as, const char* as, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line) {
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(loc, DOCTEST_COUNTOF(loc), "%s(%d)", fileForOutput(file),
                     lineForOutput(line));
...
void logAssertNothrow(bool threw, const String& exception, const char* expr, assertType::Enum assert_type, const char* file, int line) {
    char loc[DOCTEST_SNPRINTF_BUFFER_LENGTH];
    DOCTEST_SNPRINTF(loc, DOCTEST_COUNTOF(loc), "%s(%d)", fileForOutput(file),
                     lineForOutput(line));
...
ResultBuilder::ResultBuilder(assertType::Enum assert_type, const char* file, int line, const char* expr, const char* exception_type)
    : m_assert_type(assert_type), m_file(file), m_line(line), m_expr(expr), m_exception_type(exception_type), m_threw(false), m_threw_as(false), m_failed(false) {
#if DOCTEST_MSVC
