...
TEST_CASE("limits  safe subtract  size t minimum minus minimum  minimum", "[limits]") {
    REQUIRE(safe_subtract(minimum, minimum) == minimum);
}

TEST_CASE("limits  safe subtract  size t maximum minus maximum  minimum", "[limits]") {
    REQUIRE(safe_subtract(maximum, maximum) == minimum);
}

TEST_CASE("limits  safe subtract  size t maximum minus minimum  maximum", "[limits]") {
    REQUIRE(safe_subtract(maximum, minimum) == maximum);
}

TEST_CASE("limits  safe subtract  size t minimum minus maximum  throws underflow", "[limits]") {
    REQUIRE_THROWS_AS(safe_subtract(minimum, maximum), std::underflow_error);
}

TEST_CASE("limits  safe subtract  size t half minus maximum  throws underflow", "[limits]") {
    REQUIRE_THROWS_AS(safe_subtract(half, maximum), std::underflow_error);
}

// safe_increment
//-----------------------------------------------------------------------------

TEST_CASE("limits  safe increment  size t minimum  expected", "[limits]") {
    auto value = minimum;
...
TEST_CASE("limits  safe increment  size t half  expected", "[limits]") {
    auto value = half;
...
TEST_CASE("limits  safe increment  size t maximum  throws overflow", "[limits]") {
    auto value = maximum;
...
TEST_CASE("limits  safe decrement  size t maximum  expected", "[limits]") {
    auto value = maximum;
...
TEST_CASE("limits  safe decrement  size t half  expected", "[limits]") {
    auto value = half;
    static auto const expected = half - 1u;
    safe_decrement(value);
    REQUIRE(value == expected);
...
TEST_CASE("limits  safe decrement  size t minimum  throws underflow", "[limits]") {
    auto value = minimum;
...
TEST_CASE("limits  safe signed  min int64 to int32  throws range", "[limits]") {
    REQUIRE_THROWS_AS(safe_signed<int32_t>(min_int64), std::range_error);
}
...
TEST_CASE("limits  safe to signed  max uint32 to int32  throws range", "[limits]") {
    REQUIRE_THROWS_AS(safe_to_signed<int32_t>(max_uint32), std::range_error);
}
...
TEST_CASE("limits  safe to unsigned  min int32 to uint32  throws range", "[limits]") {
    REQUIRE_THROWS_AS(safe_to_unsigned<uint32_t>(min_int32), std::range_error);
}
...
TEST_CASE("limits  safe to unsigned  min int64 to uint32  throws range", "[limits]") {
    REQUIRE_THROWS_AS(safe_to_unsigned<uint32_t>(min_int64), std::range_error);
}
